...
object PayoutCalculator {
    ...
    data class Payout(val place: Int, val amount: Int)
    ...
    fun calculate(state: TournamentState): List<Payout> {
        ...
        if (pool <= 0) return percents.indices.map { Payout(it + 1, 0) }

        val sum = percents.sum().takeIf { it > 0 } ?: 100
        val rawAmounts = percents.map { pct -> (pool.toLong() * pct / sum).toInt() }
